...
/* 	       tree); */
  ...
  Generic_Brent(param,
		ax,bx,cx,1.e-10,
		param,tree,n_max_iter, lk_other_vec, maximum);
		 ...
		 arbre *tree, double * lk_other_vec, int maximum)
{
   ...
   *fa=-Return_Lk(tree, lk_other_vec, maximum);
   *param = *bx;
   if(*param > lim_sup) *param = lim_sup;
   if(*param < lim_inf) *param = lim_inf;
   *fb=-Return_Lk(tree, lk_other_vec, maximum);
   if (*fb > *fa) {
      SHFT(dum,*ax,*bx,dum)
      SHFT(dum,*fb,*fa,dum)
   }
   ...
   *param = fabs(*cx);
   if(*param > lim_sup) *param = lim_sup;
   if(*param < lim_inf) *param = lim_inf;
   *fc=-Return_Lk(tree, lk_other_vec, maximum); 
   while (*fb > *fc) 
     {
       ...
       if ((*bx-u)*(u-*cx) > lim_inf) 
	 {
	   *param = fabs(u);
	   ...
	   fu=-Return_Lk(tree, lk_other_vec, maximum);
	   if (fu < *fc) 
	     {
	       *ax=(*bx);
	       *bx=u;
	   ...
	   if (fu < *fc) 
	     {
	       ...
	       *param = fabs(u); 
	       SHFT(*fb,*fc,fu,-Return_Lk(tree, lk_other_vec, maximum))
	     }
	 } 
			  ...
			  double *xmin, arbre *tree, allseq *alldata, double *param, int n_iter_max, double * lk_other_vec, int maximum)
{
   ...
   Lk(tree,alldata, lk_other_vec, maximum);
   f1=-tree->tot_loglk;
   (*param)=x2;
		     ...
		     double *xmin, edge *b_fcus, arbre *tree, double * lk_other_vec, int maximum)
{
   ...
   b_fcus->l=x1;
   f1 = -Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
   b_fcus->l=x2;
   f2 = -Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
   while (fabs(x3-x0) > tol*(fabs(x1)+fabs(x2))) 
     {
       if (f2 < f1) 
	 {
	   SHFT3(x0,x1,x2,GOLDEN_R*x1+GOLDEN_C*x3)
	   b_fcus->l=x2;
	   SHFT2(f1,f2,-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum))
	 } 
       else 
	 {
	   SHFT3(x3,x2,x1,GOLDEN_R*x2+GOLDEN_C*x0)
	   b_fcus->l=x1;
	   SHFT2(f2,f1,-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum))
	 }
     }
   if (f1 < f2) 
     {
		...
		edge *b_fcus, arbre *tree, double * lk_other_vec, int maximum)
{
   ...
   b_fcus->l = *ax;
   *fa=-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
   b_fcus->l = *bx;
   *fb=-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
   if (*fb > *fa) {
      SHFT(dum,*ax,*bx,dum)
      SHFT(dum,*fb,*fa,dum)
   }
   *cx=(*bx)+MNBRAK_GOLD*(*bx-*ax);
   b_fcus->l = fabs(*cx);
   *fc=-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
   while (*fb > *fc) 
     {
	 ...
	 {
	   b_fcus->l = fabs(u);
	   fu=-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
	   if (fu < *fc) 
	     {
	       *ax=(*bx);
	       *bx=u;
	   ...
	   if (fu < *fc) 
	     {
	       SHFT(*bx,*cx,u,*cx+MNBRAK_GOLD*(*cx-*bx))
	       b_fcus->l = fabs(u); 
	       SHFT(*fb,*fc,fu,-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum))
	     }
	 } 
		    ...
		    double *xmin, edge *b_fcus, arbre *tree, int n_iter_max, double * lk_other_vec, int maximum)
{
  ...
  x=w=v=bx;
  b_fcus->l = fabs(bx);
  fw=fv=fx=-Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
  ...
  for(iter=1;iter<=BRENT_ITMAX;iter++)
	...
	{
	  *xmin=x;
	  Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
	  return -fx;
	}
...
void Round_Optimize(arbre *tree, allseq *data, int write_int, int read_int, int maximum)//, int read_int_end)
{
  ...
  double *lk_other_vec; //The likelihood vector which is the sum of the other trees likelihoods
 
   
  //Listening to the user signal 1 in case the server tells to stop the computations
  check_sigusr2();
  ...
  */
  ...
  do
    {
      //    printf("tree->tot_loglk before Optimize_Br_Len_Serie : %f\n",tree->tot_loglk);
        ...
        Optimize_Br_Len_Serie(root,
                              ...
                              tree,
                              data,
                              5, lk_other_vec, maximum);
    
        tree->mod->s_opt->opt_bl = 0;
        tree->both_sides         = 1;
        
	//        printf("tree->tot_loglk After Optimize_Br_Len_Serie : %f -> %f->",lk_old, tree->tot_loglk);
        Lk(tree,data, lk_other_vec, maximum);
	//	printf("AGAIN %f\n", tree->tot_loglk);
	if(!each)
            {
	...
	//	printf("tree->tot_loglk AFTER2 opt : %f -> %f\n",lk_old, tree->tot_loglk);
        lk_new = tree->tot_loglk;
 
        if(tree->mod->s_opt->print)
            {
	      /*     if(lk_old < UNLIKELY+1) 
		//write(write_int, &(tree->tot_loglk), sizeof(double));
		printf("\n. Log(lk) :               * -> %15.6f ",lk_new);
	      else
        ...
        if(lk_new < lk_old - MIN_DIFF_LK) {
	  Exit("\n. Optimization failed ! (Round_Optimize)\n");
	}
...
void Round_Optimize_End(arbre *tree, allseq *data, int write_int, int read_int, int maximum)
{
  ...
  while(n_round < ROUND_MAX)
	...
	//After a new vector has been received, we compute the new likelihood and update the vectors accordingly
	/*Lk(tree,data, lk_other_vec);
	  lk_new = tree->tot_loglk;
	  lk_old = tree->tot_loglk;
	*/
			   ...
			   arbre *tree,allseq *alldata, int n_passes, double * lk_other_vec, int maximum)
{
  ...
  //printf("\t\tlkinit before Br_Len_Brent : %f\n", lk_init);
  Br_Len_Brent(l_infa,l_max,l_infb,
               1.e-5,
               &(b_fcus->l),
               b_fcus,tree,1000, lk_other_vec, maximum);
  //if (lk_init>tree->tot_loglk) { printf("\n\n\t\tlk after Br_Len_Brent : %f->%f\n", lk_init, tree->tot_loglk);}
...
/* 	 b_fcus->l); fflush(NULL); */
  ...
  if(d->tax) return;
  else For(i,3) if(d->v[i] != a)
    {
      ...
      Optimize_Br_Len_Serie(d,d->v[i],d->b[i],tree,alldata,n_passes, lk_other_vec, maximum);
    }
...
double Br_Len_NR(arbre *tree,allseq *alldata, edge *b_fcus, int n_passes, double * lk_other_vec, int maximum)
{
  ...
  for(;;)
    {      
      lk_new = Lk_At_Given_Edge(tree,b_fcus, lk_other_vec, maximum);
...
void Moving_Backward(arbre *tree, allseq *alldata, 
		     double **step, double lk_base, double *l_branch_init, double * lk_other_vec, int maximum)
{
  double lk_new;
  int i,n_iter_max;
	  ...
	  l_infb = -10.;
          ...
          //          lk =
            Br_Len_Brent(l_infa,l_max,l_infb,
                         1.e-5,
                         &(b->l),
                         b,tree,1000, lk_other_vec, maximum);
	  b->ql[0] = b->l;
	  b->best_conf = 1;
	  b->l = l_init;
	}
...
void Optimiz_All_Free_Param(arbre *tree, int verbose, double * lk_other_vec, int maximum)
{
        ...
        if(failed)
            {
                ...
                For(i,tree->mod->n_diff_rr_param) 
                    if(i != 5) 
                        Optimize_Single_Param_Generic(tree,&(tree->mod->rr_param_values[i]),tree->mod->rr_param_values[i],1.E-20,1.E+10,1000, lk_other_vec, maximum);
            }
  ...
  if(tree->mod->s_opt->opt_kappa)
    {
      ...
      Optimize_Single_Param_Generic(tree,&(tree->mod->kappa),tree->mod->kappa,0.1,100,100, lk_other_vec, maximum);
      /* 	      printf("kappa = %f\n",tree->mod->kappa); */
  ...
  if(tree->mod->s_opt->opt_lambda) 
    {
      Optimize_Single_Param_Generic(tree,&(tree->mod->lambda),tree->mod->lambda,0.001,100,50, lk_other_vec, maximum);
      /* 	      printf("lambda = %f\n",tree->mod->lambda); */
    }
  ...
  if(tree->mod->s_opt->opt_pinvar) 
    {
      ...
      tree->mod->pinvar = 0.5;
      Optimize_Single_Param_Generic(tree,&(tree->mod->pinvar),tree->mod->pinvar,0.0001,0.9999,100, lk_other_vec, maximum);            
/*       printf("p-invar = %f\n",tree->mod->pinvar); */
    }
  ...
  if(tree->mod->s_opt->opt_alpha) 
    { 
      ...
      Optimize_Single_Param_Generic(tree,&(tree->mod->alpha),tree->mod->alpha,0.01,100,100, lk_other_vec, maximum);
  ...
  if(tree->mod->s_opt->opt_bfreq)
    {
        ...
        if(failed)
            {
                For(i,5) 
                    Optimize_Single_Param_Generic(tree,&(tree->mod->pi[i]),tree->mod->pi[i],1.E-10,0.999999,1000, lk_other_vec, maximum);                
            }
        tree->mod->update_eigen = 0;
    }
  ...
  *check=0;
  ...
  for (;;) 
      ...
      for(i=0;i<n;i++)
	{
      ...
      if(i==n) 
	{
	  *f=Return_Abs_Lk(tree, lk_other_vec, maximum);
/* 	  printf("loglk = %f\n",*f); */
	}
...
/* 	       tree); */
  ...
  Hmm_Lambda_Brent(tree, param,
		ax,bx,cx,1.e-10,
		   param,vectors,n_max_iter, n_trees);
			...
			double *xmin, double** vectors, int n_iter_max, int n_trees)
{
  ...
  *param=bx;
   ...
   tree->tot_loglk = Compute_HMM_Multi_Likelihood (tree, vectors, n_trees);
     //Lk(tree,tree->data, lk_other_vec, maximum); 
