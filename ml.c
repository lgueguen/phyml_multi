  ...
  For(site,tree->n_pattern)
    {
      ...
      tree->site_lk[site]          = .0;
      tree->curr_site              = site;
      Site_Lk(tree,alldata, lk_other_vec[site], maximum);
    }
...
/*   qsort(tree->tot_loglk_sorted,tree->n_pattern,sizeof(double),Sort_Double_Decrease); */
  ...
  if (tree->input->HMM) {
    tree->tot_loglk = Compute_HMM_Multi_Likelihood (tree, tree->mod->all_lk_vec, tree->input->n_trees); 
    // printf("HMM : tree->tot_loglk : %f\n", tree->tot_loglk);
  }
  else {
    For(site, tree->n_pattern)
      {
 ...
 if (!tree->input->HMM) {
   if (maximum) {
	 ...
	 if ((double)tree->data->invar[site] > -0.5)
	   {
	     if (site_lk*(1.0-tree->mod->pinvar) + tree->mod->pinvar*tree->mod->pi[state_elsewhere]>lk_other) {
	   ...
	   {
	     if(site_lk*(1.0-tree->mod->pinvar)>lk_other) {
     ...
     if (!tree->mod->invar)
       {
	 log_site_lk = log(site_lk+lk_other); 
	 ...
	 if ((double)tree->data->invar[site] > -0.5)
	   {
	     //	  if (!(eroot->sum_scale_f_left[site] + eroot->sum_scale_f_rght[site]==0.0))
	     //  site_lk *= exp(eroot->sum_scale_f_left[site] + eroot->sum_scale_f_rght[site]);
	     log_site_lk = log(site_lk*(1.0-tree->mod->pinvar) + tree->mod->pinvar*tree->mod->pi[state_elsewhere]+lk_other);
	   }
	 else
	   {
	     log_site_lk = log(site_lk*(1.0-tree->mod->pinvar)+lk_other);
	     // + eroot->sum_scale_f_left[site] + eroot->sum_scale_f_rght[site];
	   }
       }
   }
  ...
  tree->true_site_lk[site] = site_lk;
      ...
      if(tree->data->wght[site])
	{
	  ...
	  if (! tree->input->HMM) {
	    if (maximum){
	      if (!tree->mod->invar)
		{
		  if(site_lk > lk_other_vec[site]){
		    log_site_lk += log(site_lk) ;
		  }
		  else {
		    log_site_lk += log(lk_other_vec[site]) ;
		  }
		}
	      else {
		if ((double)tree->data->invar[site] > -0.5)
		  {
		    if(site_lk*(1.0-tree->mod->pinvar) + tree->mod->pinvar*tree->mod->pi[tree->data->invar[site]]>lk_other_vec[site]) {
		  ...
		  if(site_lk*(1.0-tree->mod->pinvar)> lk_other_vec[site]) {
		    log_site_lk = log(site_lk*(1.0-tree->mod->pinvar)); 
		  }
		  else {
		    log_site_lk =lk_other_vec[site]; 
		  }
		}
	      }
	    }
	      ...
	      if (!tree->mod->invar)
		{
		  ...
		  log_site_lk += log(site_lk+lk_other_vec[site]) ;
		  ...
		  if ((double)tree->data->invar[site] > -0.5)
		    {
		      ...
		      log_site_lk = log(site_lk*(1.0-tree->mod->pinvar) + tree->mod->pinvar*tree->mod->pi[tree->data->invar[site]]+lk_other_vec[site]);
		    }
		  else
		    {
		      log_site_lk = log(site_lk*(1.0-tree->mod->pinvar)+lk_other_vec[site]); 
		      //		    + b_fcus->sum_scale_f_left[site] + b_fcus->sum_scale_f_rght[site];
		    }
		}
	    }
...
/*   qsort(tree->tot_loglk_sorted,tree->n_pattern,sizeof(double),Sort_Double_Decrease); */
  ...
  if(tree->input->HMM) {
    tree->tot_loglk = Compute_HMM_Multi_Likelihood (tree, tree->mod->all_lk_vec, tree->input->n_trees); 
    //  printf("Client %d : HMM Lk_At_Given_Edge : tree->tot_loglk : %f\n", getpid(), tree->tot_loglk);
  }
  else {
...
void Make_Tree_4_Lk(arbre *tree, allseq *alldata, int n_site)
{
  ...
  if(!tree->true_site_lk)     tree->true_site_lk     = (double *)mCalloc(alldata->crunch_len,sizeof(double));
...
double Compute_Real_Multi_Likelihood (arbre *tree, double** vectors, int n_trees, int maxi)
{
  ...
  lk_summary = (double *)mCalloc(tree->data->crunch_len,sizeof(double));
  ...
  if (maxi) { //We choose to take the most likely topology at each site
	...
	if (vectors[i][site] > lk_summary[site]) {
      ...
      lk_summary[site] = log(lk_summary[site]);
      likelihood+= tree->data->wght[site]*lk_summary[site];
    }
    return likelihood;
  }
  else { //We choose to take the average over the topologies at each site
    For(site, tree->data->crunch_len) {
      lk_summary[site]=0.0;
      For(i,n_trees){
	lk_summary[site]+=vectors[i][site];
      }
      lk_summary[site] = lk_summary[site]/n_trees;
      lk_summary[site] = log(lk_summary[site]);
      likelihood+= tree->data->wght[site]*lk_summary[site];
    }
    return likelihood;
...
/*********************************************************/
//If average, we do not take the average but the sum ; if max, it is still the same as just above.
double Compute_Simplified_Multi_Likelihood (arbre *tree, double** vectors, int n_trees, int maxi)
{
  ...
  else { //We choose to take the sum over the topologies at each site (shortcut for average)
    For(site, tree->data->crunch_len) {
      lk_summary[site]=0.0;
      For(i,n_trees){
	lk_summary[site]+=vectors[i][site];
...
//Compute the likelihood according to Felsenstein and Churchill, MBE 1996, forward algorithm
//As the Markov process is reversible, forward and backward can use the same transition probabilities 
double Compute_HMM_Multi_Likelihood (arbre *tree, double** vectors, int n_trees)
{
  ...
  double **fb_lk;
  double sum = 0;
  double *scaler ; //table to avoid scaling problems
  double lambda = tree->mod->hmm_lambda;
  //  printf ("LAMBDA in HMM : %f\n", lambda);

  lk_summary = (double *)mCalloc(tree->data->crunch_len,sizeof(double));

  scaler = (double *)mCalloc(tree->data->crunch_len,sizeof(double));

  //allocating the matrix for the computation of the likelihood of the model
  fb_lk = (double **)mCalloc(n_trees,sizeof(double*));
  For(i,n_trees){
    fb_lk[i] = (double *)mCalloc(tree->data->crunch_len,sizeof(double));
  }
  ...
  For(site, tree->data->crunch_len) {
    if (site==0) {
      sum=0;
      For(i,n_trees){
	//fb_lk[i][site] = exp(vectors[i][site]);
	fb_lk[i][site] = vectors[i][site];
	sum = sum + fb_lk[i][site];
      }
      scaler[site] = 1/sum;//scaling
      For(i,n_trees){
	fb_lk[i][site]=scaler[site]*fb_lk[i][site];
      }
    }
    else {
      For(j,n_trees){ //going through the states at position site
	sum = 0;
	For(i,n_trees){ //going through the states at position site-1
	  if (j==i){
	    sum = sum + fb_lk[i][site-1] * lambda + fb_lk[i][site-1] * ((1-lambda)/n_trees);
	  }
	  else {
	    sum = sum + fb_lk[i][site-1] * ((1-lambda)/n_trees);
	  }
	}
	//	fb_lk[i][site] = exp(vectors[i][site]) * sum;
	fb_lk[j][site] = vectors[j][site] * sum;
      }
      sum=0;
      For(i,n_trees){ 
	sum=sum+fb_lk[i][site];
      ...
      scaler[site] =  1/sum;//scaling
      For(i,n_trees){ 
	fb_lk[i][site] = fb_lk[i][site] * scaler[site];
      }
    }
  }
...
int* Compute_Viterbi (arbre *tree, double** vectors, int n_trees) {
  ...
  argmax = (int **)mCalloc(tree->data->crunch_len,sizeof(int*));
  For(i,n_trees){
    argmax[i] = (int *)mCalloc(tree->data->crunch_len,sizeof(int));
  }
  toreturn = (int *)mCalloc(tree->data->crunch_len,sizeof(int));

  
  //allocating the matrix for the computation of the likelihood of the model
  vit_lk = (double **)mCalloc(n_trees,sizeof(double*));
  For(i,n_trees){
    vit_lk[i] = (double *)mCalloc(tree->data->crunch_len,sizeof(double));
  }
  ...
  For(site, tree->data->crunch_len) {
    ...
    if (site==0) {
      For(i,n_trees){
	vit_lk[i][site] = log(vectors[i][site]);
	//if (vit_lk[i][site]>maxi) {
	//maxi=vit_lk[i][site];
	  argmax[i][site]=i;
	  //}
      }
	...
	For(i,n_trees){ //going through the states at position site-1
	  if (j==i){
	    if (log(lambda + ((1-lambda)/n_trees))+vit_lk[i][site-1]>maxi) {
	      maxi = log(lambda + ((1-lambda)/n_trees))+vit_lk[i][site-1];
	      argmax[j][site] = i;
	    }
	  }
	  else {
	    if (log((1-lambda)/n_trees)+vit_lk[i][site-1]>maxi) {
	      maxi = log((1-lambda)/n_trees)+vit_lk[i][site-1];
	      argmax[j][site] = i;
	    }
	  }
	}
	vit_lk[j][site] = maxi + log(vectors[j][site]);
      }
    }
  }
  ...
  //Backtracking
  //Initiating the way backwards
  site=tree->data->crunch_len-1;
   maxi=UNLIKELY;
   For(j,n_trees){
    if (vit_lk[j][site]>maxi) {
      maxi = vit_lk[j][site];
      toreturn[site] = j;
    }
  }
  ...
  for(site=tree->data->crunch_len-1; site>0; site--) {
    toreturn[site-1] = argmax[toreturn[site]][site];
  }
