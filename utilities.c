...
int Read_One_Line_Seq(seq ***data, int num_otu, FILE *in)
{
  ...
  while(1)
    {
      if((c == EOF) || (c == '\n')) break;
      else if((c==' ') || (c=='\t') || (c=='\r')) {c=(char)fgetc(in); continue;}
      Uppercase(&c);
...
allseq *Compact_Seq(seq **data, option *input)
{
  ...
  Fors(site,data[0]->len,input->mod->stepsize) 
    { 
      ...
      //As we use a first filtering step that potentially removes unnecessary trees and uses HMM, we never compact the sequences
...
allseq *Compact_CSeq(allseq *data, model *mod)
{
  ...
  alldata->n_otu=n_otu;
  alldata->clean_len=data->clean_len;
...
void NNI(arbre *tree, edge *b_fcus, int do_swap, double * lk_other_vec, int maximum)
{
  //  int r_l, l_v1 l_v2, r_v3, r_v4, l_r, 
  node *v2,*v3,*v4;//*v1,
  double lk1, lk2, lk3;
  ...
  v2 = v3 = v4 = NULL;//v1 = NULL;
  //  l_v1 = l_v2 =  r_v3 = r_v4 = -1;
  ...
  tree->both_sides    = 1;
  lk2_init = Update_Lk_At_Given_Edge(b_fcus,tree, lk_other_vec, maximum);
  ...
  tree->mod->s_opt->opt_bl = 0;
  tree->both_sides = 1;
  lk3_init = Update_Lk_At_Given_Edge(b_fcus,tree, lk_other_vec, maximum);
...
void Update_All_Partial_Lk(edge *b_fcus, arbre *tree, double * lk_other_vec, int maximum)
{
...
model *Copy_Model(model *ori)
{
  ...
  For(i,cpy->ns) cpy->pi[i] = ori->pi[i];
...
double* Compute_Lk_Summary (arbre *tree, double** vectors, int n_trees, int maxi, int current_tree)
{
  int site, i;
  double *lk_summary;
...
/*********************************************************/
void Read_From_Memory (char * fname, double **lkmat, int lineNum, int colNum) {
  int fd, i, j;
  double *lkvec;
...
/*********************************************************/
void Read_From_Memory_With_Lambda (char * fname, double **lkmat, double *lambda, int lineNum, int colNum) {
  int fd, i, j;
  double *lkvec;
...
void Update_Lk_Vector_In_Lk_Table (arbre *tree) {
  int site;
  For (site, tree->data->crunch_len) {
    tree->mod->all_lk_vec[tree->input->current_tree][site] = tree->true_site_lk[site];
  }
  return;
}
...
seq **Get_Seq_MultiTrees(option *input,  int rw)
{
  seq **data;
  int i,j;
  char **buff;
  int n_unkn,n_removed,pos;
  int *remove;
  int cur_tree=input->current_tree;
  int n_trees=input->n_trees;
  int segLength;
  int seg_start;
  int seg_end;
  ...
  //We need to close and re-open the sequence file 
  ...
  printf ("in Get_Seq_MultiTrees2\n");
  
  if(data)
    {
      printf ("in Get_Seq_MultiTrees3\n");
      segLength = data[0]->len/n_trees;
      buff = (char **)mCalloc(input->mod->n_otu,sizeof(char *));
      For(i,input->mod->n_otu) buff[i] = (char *)mCalloc(segLength,sizeof(char));
      remove = (int *)mCalloc(segLength,sizeof(int));
  
      n_removed = 0;

      seg_start = cur_tree * segLength;
      seg_end = seg_start + segLength;
