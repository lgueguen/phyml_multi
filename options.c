...
#define N_NB_PARAMS_DNA 15
#define N_NB_PARAMS_AA 14
...
void Get_Input_CommandLine_Common(option *input, int argc, char **argv)
{
  ...
  p = argv[N_HMM];
  input->HMM = (!strcmp(p,"y")) ? 1 : 0;

  p = argv[N_TREES];
  if (!atoi(p) || (input->n_trees = atoi(p)) < 0)
    Exit("\nThe number of trees should be a positive integer\n");
