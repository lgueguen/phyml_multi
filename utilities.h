...
typedef struct __Arbre {
    ...
    double                        *true_site_lk; //site_lk is in reality log_site_lk
...
typedef struct __AllSeq {
  ...
  int        init_len;
  int         *ambigu;
  int       **pospatt;             /* gives the site positions corresponding to the patterns */
}allseq;
...
*/
  ...
  double   hmm_lambda; /*autocorrelation parameter, 1-hmm_lambda is the probability to change between hidden states (trees), as in Felsenstein and Churchill MBE 1996*/
  int message_size; /*size of the vector that is exchanged between the server and the clients*/

  double ** all_lk_vec; //table containing all the likelihood vectors from all the trees
...
typedef struct __Option {
  ...
  int                         HMM;
  char                     *fname;
  int                current_tree; //tells each client what is the index of the tree they manage
  int                        maxi; //Shall we use the maximum or the average
}option;
