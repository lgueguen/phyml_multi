...
void Init_Model(allseq *data, model *mod)
{
  ...
  if(!mod->datatype) /* Nucleotides */
      ...
      if(mod->whichmodel < 40)
	{
	  ...
	  if((mod->whichmodel==1) || (mod->whichmodel==3) || (mod->whichmodel==7))
	    {
	      mod->kappa = 1.;
	    }
	  ...
	  if(mod->whichmodel == 8)
		...
		For(i,4) mod->pi[i] = mod->user_b_freq[i];
	      Update_Qmat_GTR(mod);
	    }
  ...
  mod->hmm_lambda = 0.99; //Initial expected state length : 1/(1-lambda)=100
